...
import { validateStoreProfileFields, type StoreProfileFormErrors } from "@/lib/store-profile";
import {
...
function getEmailFromCookie() {
  ...
  const cookieName = CUSTOMER_AUTH_COOKIES.user.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
export default function StoreAccountSettingsClient() {
  ...
  const { data: accountResponse, isLoading: accountLoading } = useStoreAccountDetails(loggedIn);
  const { data: addressesResponse, isLoading: addressesLoading } = useStoreAddresses(loggedIn);
  const upsertAccount = useUpsertStoreAccountDetails();
  const upsertAddress = useUpsertStoreAddress();

  useEffect(() => {
    if (!hasUserSession()) {
      window.location.href = buildConnectLoginUrl("/account");
      return;
    }
  ...
  useEffect(() => {
    ...
    setHasHydrated(true);
  }, [accountResponse, addressesResponse, accountLoading, addressesLoading, hasHydrated]);
  ...
  if (isCheckingAuth || !hasHydrated) {
    return <div className="container mx-auto px-4 py-16 h-64 animate-pulse bg-gray-50 rounded-2xl" />;
  }

  return (
    <div className="container mx-auto max-w-4xl px-4 py-12">
      <div className="mb-8 space-y-2">
        <p className="text-xs font-bold uppercase tracking-widest text-red-600">Store account</p>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              These details are auto-filled at checkout after you save them once.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    setAccountForm((prev) => ({ ...prev, full_name: e.target.value }));
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  disabled={accountLoading}
                  onChange={(e) => {
                    setAccountForm((prev) => ({ ...prev, phone_number: e.target.value }));
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled={accountLoading}
                onChange={(e) =>
                  setAccountForm((prev) => ({ ...prev, organization_name: e.target.value }))
                }
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                {formErrors.gst_number ? (
                  <p className="mt-1 text-xs text-red-600">{formErrors.gst_number}</p>
                ) : null}
              </div>
              <div className="space-y-2">
                ...
                {formErrors.pan_number ? (
                  <p className="mt-1 text-xs text-red-600">{formErrors.pan_number}</p>
                ) : null}
              </div>
            </div>
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Select
                ...
                onChange={(e) => setBillingForm((prev) => ({ ...prev, state_id: e.target.value }))}
              >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Leave empty if you use billing address at checkout.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            <Textarea
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Select
                ...
                disabled={statesLoading}
                onChange={(e) => setShippingForm((prev) => ({ ...prev, state_id: e.target.value }))}
              >
                <option value="">Select state</option>
                {states.map((state) => (
                  <option key={state.id} value={state.id}>
